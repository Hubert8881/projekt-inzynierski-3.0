...
const reservationSchema = z.object({
  ...
  party_size: z.number().int().min(1).max(20).optional().default(1),
  customer_first_name: z.string().min(2).max(50).trim(),
  customer_last_name: z.string().min(2).max(50).trim(),
  customer_email: z.string().email().trim(),
  customer_phone: z.string().min(9).max(15).trim()
});
...
export class ReservationController {
  ...
  static async createReservation(req: Request, res: Response): Promise<void> {
    try {
      ...
      const cleanData = sanitizeObject(validatedData);
