...
class ReservationModel {
    static async getAvailableTables(restaurantId, date, time, partySize) {
      ...
      `;
            const result = await database_1.pool.query(query, [restaurantId, partySize, date, time]);
            return result.rows;
        }
        catch (error) {
    ...
    static async createReservation(reservationData) {
        ...
        try {
            ...
            let customer;
            const customerResult = await client.query('SELECT * FROM customers WHERE email = $1', [reservationData.customer_email]);
            if (customerResult.rows.length > 0) {
                customer = customerResult.rows[0];
            }
            else {
                const newCustomerResult = await client.query(`INSERT INTO customers (first_name, last_name, email, phone) 
           VALUES ($1, $2, $3, $4) RETURNING *`, [
            ...
            const availableTables = await this.getAvailableTables(reservationData.restaurant_id, reservationData.reservation_date, reservationData.reservation_time, reservationData.party_size);
            if (availableTables.length === 0) {
                throw new Error('No available tables for the requested time and party size');
            }
            const selectedTable = availableTables[0];
            const reservationResult = await client.query(`INSERT INTO reservations 
    ...
    static async getReservationsByDate(restaurantId, date) {
      ...
      `;
            const result = await database_1.pool.query(query, [restaurantId, date]);
            return result.rows;
        }
        catch (error) {
